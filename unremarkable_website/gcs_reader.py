...
def check_pages(page_exts):
    ...
    all_pages_jpg = all([e.lower() in ['jpg','jpeg','png'] for e in page_exts.values()])
...
def check_artifact_readiness(gcs_client):
    ...
    # get pages
    pages = [b.name for b in gcs_client.list_blobs(PAGE_BUCKET)]
    artifact_info['pages'] = pages

    print('Found these pages:')
    print(str(pages))
